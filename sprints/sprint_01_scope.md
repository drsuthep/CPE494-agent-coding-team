...
Sprint 1 intentionally uses direct per-entity Customer views and templates because shared form/list modules are explicitly out of scope. Future sprints must extract or reuse shared modules before adding more real entities.
...
|---|---|---|---|
| `id` | BigAutoField | auto | Django default PK from `default_auto_field`; hidden from UI |
...
The Sprint 1 customer list-placeholder is exempt from shared list partial, table, pagination, search, sort, filter, and `.list-viewport` requirements. Those requirements apply when the real list view ships.
...
- `templates/customers/form.html` - extends `base.html`.
  ...
  - The delete confirmation modal submits a POST form to the button's `data-url` and includes `{% csrf_token %}`. Confirm text includes the customer display code.
...
- `templates/placeholders/invoices_list.html` - same shape as products_list but titled "Invoices", with + linking to `/invoices/new/` and X linking to `/`.
...
7. `POST /customers/new/` with valid `code` and `name` returns 302 to `/customers/<id>/edit/`. When the redirect is followed, the GET response contains the substring "Save successful" (Django messages flash text).
...
1. `test_customer_model_fields` - `Customer._meta.get_fields()` includes all expected fields with correct types and constraints (`id` is a Django auto PK, `code` is unique, `name` is required, `notes` is TextField).
...
19. `test_no_thai_rendered_pages` - `GET /`, `/customers/`, `/customers/new/`, `/products/`, `/products/new/`, `/invoices/`, and `/invoices/new/` responses contain no Unicode chars in U+0E00-U+0E7F.
20. `test_no_thai_generated_files` - scan generated Python and template files for Sprint 1 (`customers/**/*.py`, `erp_invoice/urls.py`, `erp_invoice/settings.py`, `templates/**/*.html`) and assert no Unicode chars in U+0E00-U+0E7F.
...
- All 20 test cases pass via python manage.py test.
